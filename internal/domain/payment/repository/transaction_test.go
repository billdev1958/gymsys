...
func setupTestDB(t *testing.T) *pgxpool.Pool {
	...
	if err != nil {
		t.Fatalf("failed to seed subscription cost: %v", err)
	}

	return dbPool
}

func TestPaymentRepository_RegisterUser(t *testing.T) {
	dbPool := setupTestDB(t)
	defer dbPool.Close() // Asegura que la conexión se cierre después de las pruebas

	paymentRepo := repository.NewPaymentRepository(&postgres.PgxStorage{DBPool: dbPool})
	...
	t.Run("Success", func(t *testing.T) {
		...
		userID, err := paymentRepo.RegisterUser(ctx, register)
		if err != nil {
	...
	t.Run("Fail_InsertUser", func(t *testing.T) {
		...
		_, err := paymentRepo.RegisterUser(ctx, register)
